...
\cppf{P0099/simple}
Variable \emph{l1} is passed to stackful execution context \emph{l2} as
an argument of \emph{l2}. \emph{l1} represents the suspended context that has
resumed \emph{l2}. The stack frames of \cpp{std::printf()} are allocated on the
side stack owned by \emph{l2}. On return the lambda resumed contet \emph{l1}. 
...
\newline
Experience with \cpp{execution\_context} from Boost.Context\cite{bcontext}
turned up a common pattern: to pass a lambda to the \cpp{execution\_context},
...
\emph{this}-pointer of \cpp{X}. The body converts an integer variable (input)
