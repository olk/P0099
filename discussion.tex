...
\graphc{cycle}
The cycle is started by calling \cpp{ctx3()} (1. resume) in the main context.
...
\cpp{(*other)()}. In \emph{ctx3} \cpp{ctx2()} returns and \cpp{ctx1()} resumes
\emph{ctx1} at next. Context \emph{ctx1} terminates after returning from
\cpp{(*other)()}. After termination of \emph{ctx1}, \emph{ctx3} is resumed
because it has become the parent context of \emph{ctx1}) by calling \cpp{ctx1()}.
