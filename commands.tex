\setlength{\parindent}{0pt} 
\renewcommand\sfdefault{phv}

\makeatletter
    \renewcommand*\l@subsection{\@dottedtocline{2}{0em}{2.3em}}
    \renewcommand*\l@subsection{\@dottedtocline{3}{0em}{3.2em}}
    \renewcommand{\tableofcontents}{%
        \@starttoc{toc}
    }
\makeatother

%\renewcommand{\thesubsection}{\Roman{subsection}}

\newcommand{\pdfimg}[1]{\pdfximage{pics/#1}\pdfrefximage\pdflastximage}
\newcommand{\img}[1]{\mbox{\pdfimg{#1}}}
\newcommand{\imgc}[1]{\begin{center}\img{#1}\end{center}}

\newcommand{\cpp}[1]{
    \lstinline[
        language=C++,
        basicstyle=\ttfamily\color{black},
        keywordstyle=\color{blue},
        commentstyle=\color{green},
        stringstyle=\color{red},
        morekeywords={await,yield,stackfull}
    ] {#1}
}
\newcommand{\cppf}[1]{
    \lstinputlisting[
        language=C++,
        basicstyle=\ttfamily\color{black},
        keywordstyle=\color{blue},
        commentstyle=\color{red},
        stringstyle=\color{green},
        morekeywords={await,yield,stackfull}
    ] {code/#1}
}

\newcommand{\ascoro}{\textit{asymmetric} coroutine\xspace}
\newcommand{\ascoros}{\textit{asymmetric} coroutines\xspace}
\newcommand{\await}{\textit{\cpp{await}}\xspace}
\newcommand{\cblock}{\textit{control-block}\xspace}
\newcommand{\coopmultitasking}{\textit{cooperative multitasking}\xspace}
\newcommand{\corofunction}{\textit{coroutine-function}\xspace}
\newcommand{\cstack}{\textit{call stack}\xspace}
\newcommand{\fiber}{\textit{fiber}\xspace}
\newcommand{\fibers}{\textit{fibers}\xspace}
\newcommand{\resumable}{\textit{resumable}\xspace}
\newcommand{\resumfn}{\textit{resumable function}\xspace}
\newcommand{\resumld}{\textit{resumable lambda}\xspace}
\newcommand{\sfanno}{\textit{stackfull[]}\xspace}
\newcommand{\sfull}{\textit{stackfull}\xspace}
\newcommand{\sfcoro}{\textit{stackfull} coroutine\xspace}
\newcommand{\sfcoros}{\textit{stackfull} coroutines\xspace}
\newcommand{\sless}{\textit{stackless}\xspace}
\newcommand{\slcoro}{\textit{stackless} coroutine\xspace}
\newcommand{\slcoros}{\textit{stackless} coroutines\xspace}
\newcommand{\stack}{\textit{stack}\xspace}
\newcommand{\sycoro}{\textit{symmetric} coroutine\xspace}
\newcommand{\sycoros}{\textit{symmetric} coroutines\xspace}
\newcommand{\uthread}{\textit{user-land} thread\xspace}
\newcommand{\uthreads}{\textit{user-land} threads\xspace}
\newcommand{\yield}{\textit{\cpp{yield}}\xspace}

\newcommand{\abschnitt}[1]{
    \addcontentsline{toc}{subsection}{#1}
    \subsection*{#1}
}

\newcommand{\uabschnitt}[1]{
    \addcontentsline{toc}{paragraph}{#1}
    \paragraph*{#1}
}
