...
{\bf Notes}
\newline
It is important that the coroutine is valid (using \cpp{operator bool()})
befor calling this function otherwise it results in undefined behaviour.\\
...
\newline
It is important that the coroutine is valid (using \cpp{operator bool()})
befor calling this function otherwise the behaviour is undefined.\\
...
{\bf Exceptions}
\begin{description}
    \item[1)] The move-assignable value was already accessed any successive call
        throws a \cpp{std::coroutine_error} until \cpp{operator()()} is called.\\
\end{description}
...
moves the value. Any successive call of \cpp{iterator::operator*()} throws an
