...
The author assumes that transforming the syntax of \ectx into a syntax similar
...
size \cpp{x} bytes. With \cpp{segmented(y)} an linked stack that grows on demand
with an initial size of \cpp{y} bytes is used.\\
...
Because the function \cpp{main()} can be interpreted as an execution context too
...
free-standing function similar to \cpp{std::execution_context::current()}.
