...
\cpp{execution_context::operator()} preserves the CPU register set + stack pointer:
the content of those registers is stored in the capture record
associated with \cpp{execution_context::current()}. Then \cpp{operator()}
loads the CPU register set and stack pointer from the capture record
associated with \cpp{*this}.
...
(called \ectxop on) the currently active \cpp{execution_context}.
