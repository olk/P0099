...
arguments other than \cpp{void}. For purposes of discussion,
consider \ectxargs (where \cpp{args...} here represents any list of type
template arguments other than \cpp{void}). The function or lambda that
...
is \emph{not-a-context}. Its \opbool returns \cpp{false};
...
\cpp{f(}\ectxvoid\cpp{\ mainctx)} and calls \cpp{fctx()}, thereby
...
a valid \cpp{execution\_context} instance (\cpp{execution\_context::operator
bool()} returns \cpp{true}) is called, the stack will be unwound and
...
The stack on which \cpp{main()}, as well as the stack on which the
\emph{thread-function} passed to \cpp{std::thread}'s constructor is executed,
...
or \cpp{operator\!()} before referencing any of the \cpp{args...} in the
...
explicitly-launched thread) the thread-function passed to \cpp{std::thread}'s
constructor.
