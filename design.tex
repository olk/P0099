...
The \corofunction returns \cpp{void} and takes its counterpart coroutine as argument.
Both coroutine types take the same template argument.\\
\cppf{corofn_signature.cpp}
...
\coro, started from the main-context, has a signature of \cpp{int(void)} - it returns an
...
\paragraph*{input iterator:}
\coro with signature \cpp{T(void)} provide input iterators.
\cppf{alternative_output_iterator.cpp}

\paragraph*{output iterator:}
\coro with signature \cpp{void(T)} provide output iterators.
