...
{\bfseries Notes}
\newline
If an exception leaves this function \cpp{std::terminate()} is called. When this
function returns, \cpp{std::exit(0)} is called. The behaviour is undefined if
\cpp{operator()} is called while \cpp{current()} returns \emph{*this} (e.g.
